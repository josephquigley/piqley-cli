...
struct ProcessCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        guard FileManager.default.fileExists(atPath: AppConfig.configPath.path) else {
            logger.error("Config not found. Run `\(AppConstants.name) setup` first.")
            throw ExitCode(1)
        }
        ...
        // Acquire lock
        let lockPath = NSTemporaryDirectory() + "\(AppConstants.name)/\(AppConstants.name).lock"
        let lock: ProcessLock
        do {
            lock = try ProcessLock(path: lockPath)
        } catch ProcessLockError.alreadyRunning {
            logger.error("Another instance of \(AppConstants.name) is already running.")
            throw ExitCode(1)
        }
        defer { lock.release() }

        // Create temp directory for resized images (separate from lock dir)
        let tempDir = NSTemporaryDirectory() + "\(AppConstants.name)/images/"
        ...
        let ghostAPIKey = try secretStore.get(key: "\(AppConstants.name)-ghost")
    ...
    private func seedEmailLog(emailLog: EmailLog, client: GhostClient, config: AppConfig) async {
        let logger = Logger(label: "\(AppConstants.name).email-seed")
        do {
            var page = 1
